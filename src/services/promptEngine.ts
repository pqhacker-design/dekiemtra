...
5. Tất cả công thức Toán học, Vật lý, Hóa học, ký hiệu (kể cả hệ phương trình \\begin{cases}...\\end{cases}, \\Delta, \\Leftrightarrow, \\in, \\forall) ở TẤT CẢ các trường (bao gồm cả 'explanation', 'essayAnswerGuide') BẮT BUỘC phải được bọc chuẩn bằng cặp dấu $...$ (ví dụ: $\\begin{cases} a = 1 \\\\ b = 2 \\end{cases}$, $\\Delta' = m^2 - 4 < 0$, $\\forall x \\in \\mathbb{R}$). Tuyệt đối KHÔNG viết các lệnh gạch chéo ngược LaTeX ngoài dấu $.
6. Trả về đúng 1 đối tượng JSON duy nhất theo đúng cấu trúc schema được yêu cầu, không có lời mở đầu hay kết luận thừa ngoài JSON.
