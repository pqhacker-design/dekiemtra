...
5. Tất cả công thức Toán học, Vật lý, Hóa học, ký hiệu ở TẤT CẢ các trường (bao gồm cả 'explanation', 'essayAnswerGuide') BẮT BUỘC phải được bọc chuẩn bằng cặp dấu $...$ (ví dụ: $\Delta' = m^2 - 4 < 0$, $\forall x \in \mathbb{R}$). ĐẶC BIỆT: HỆ PHƯƠNG TRÌNH bắt buộc dùng môi trường \\begin{cases} ... \\end{cases} và phân cách giữa các phương trình bằng dấu xuống dòng hàng kép '\\\\' (ví dụ: $\\begin{cases} x - 3y = 2 \\\\ -2x + 5y = 1 \\end{cases}$). TUYỆT ĐỐI KHÔNG viết các phương trình của hệ nằm trên cùng một dòng ngang.
